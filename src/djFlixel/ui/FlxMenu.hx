...
class FlxMenu extends FlxGroup {
	...
	**/
	public function open(hard:Bool = false) {
		...
		if (history.length == 0 || mpActive != null || isFocused)
			return;
	...
	**/
	...
	function headerText_show() {
		if (!PARAMS.header_enable)
			return;

		if (headerText == null) // :: Create it
		{
			if (pageActive.title == null)
				return; // No need to create it right now
			...
			if (stHeader.c == null)
				stHeader.c = stI.col_t.accent;
			if (stHeader.bc == null)
				stHeader.bc = stI.col_b.accent != null ? stI.col_b.accent : stI.col_b.idle;
	...
	function headerText_hide() {
		if (!PARAMS.header_enable || headerText == null)
			return;
		headerText.visible = decoLine.visible = false;
	} //---------------------------------------------------;
	...
	function on_list_event(msg:String) {
		if (msg == "back") {
			if (pageActive != null && pageActive.params.noBack)
				return;
			goBack();
		} else if (msg == "start") {
			_mev(start);
				...
				case 2: // Call - Confirm Popup
					...
					MP.onItemEvent = (ev, it2) -> {
						if (ev != fire)
							return;
						CLOSE_MP();
						if (it2.data.type == 1 && onItemEvent != null)
							onItemEvent(ev, it2);
					};
