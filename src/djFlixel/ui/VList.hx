...
typedef VListStyle = {
	...
	sind_size:Int,
	// which icon size to use. Be sure to call D.ui.initIcons(size); beforehand
	sind_anim:String,
	// Type,Steps,Time, Type(1=repeat,2=loop,3=blink), Steps=int, Time=Float=step time
	sind_color:Int,
	// Colorize the scroll indicator with this color.
	sind_offx:Int
	// Offset X axis for the arrows
}
...
class VList<T:IListItem<K> & FlxSprite, K> extends FlxSpriteGroup {
	...
	**/
	 ...
	 * @param	X Screen Pos
	 * @param	Y Screen Pos
	 * @param	MENU_WIDTH 0 To autocalculate right after `setdata()` -1 To autofill to screen width Mostly used for center/justify
	...
	override public function update(elapsed:Float):Void {
		...
		if (isFocused) {
			...
			if (!exists)
				return; // Simple solution for above  ^^^^
	...
	**/
	public function setCursor(S:OneOfTwo<FlxSprite, BitmapData>, ?OFFSET:Array<Int>, TWEEN_TIME_MULTIPLIER:Float = -1):Void {
		...
		if (Std.is(S, FlxSprite)) {
			cursor = cast S;
		} else if (Std.is(S, BitmapData)) {
		...
		if (TWEEN_TIME_MULTIPLIER < 0)
			TWEEN_TIME_MULTIPLIER = DEFAULT_CURSOR_TWEEN_MULT;
	...
	**/
	...
	function processInput() {
		if (D.ctrl.timePress(UP)) { // UP
		...
		} else if (D.ctrl.timePress(DOWN)) { // DOWN
			if (inputMode == 2) {
				selectionDown1();
			} else {
				scrollDown1();
			...
			return;
		else // Scroll Mode, is only UP/DOWN which were just checked
			...
			} else if (D.ctrl.justPressed(X)) {
				if (onListEvent != null)
					onListEvent("back");
			} else if (D.ctrl.justPressed(START)) {
				if (FLAGS.start_button_fire)
					indexItem.onInput(fire);
				else if (onListEvent != null)
					onListEvent("start");
			}
	} //---------------------------------------------------;
	...
	function scrollDown1():Bool {
		...
		if (style.scroll_time == 0) {
			...
			itemSlots[_cnt] = _itm;
			isScrolling = true;
			tween_slot.push(FlxTween.tween(_itm, {alpha: 1, y: _itm.y - itemHeight}, style.scroll_time, {onComplete: on_scrollComplete}));
		}
	...
	function selectionUp1() {
		...
		if (r_1 < 0)
			return; // Can't find a selectable item
	...
	**/
	function currentItem_Focus() {
		...
		if (style.focus_nudge != 0) {
			...
			tween_map.set(indexItem,
				FlxTween.tween(indexItem, {x: x + get_itemStartX(indexItem) + style.focus_nudge + pingPongOffset}, style.scroll_time, tweenProperties));
		}
		cursor_updatePos();
	} //---------------------------------------------------;
	...
	**/
	function currentItem_Unfocus() {
		...
		if (style.focus_nudge != 0) {
			...
			tween_map.set(indexItem, FlxTween.tween(indexItem, {x: x + get_itemStartX(indexItem)}, style.scroll_time));
		}
	} //---------------------------------------------------;
	...
	**/
	function cursor_updatePos() {
		...
		if (cur.tween_time == 0) {
			...
			tween_map.set(cursor, FlxTween.tween(cursor, {x: zeroline + style.focus_nudge, alpha: 1}, cur.tween_time, {ease: FlxEase.cubeOut}));
		}
	...
	**/
	...
	function scroll_ind_tick(v:Int) {
		if (sind_ty < 3) {
			...
			if (get_hasMoreDown())
				sind_el[1].visible = !sind_el[1].visible;
			if (get_hasMoreUp())
				sind_el[0].visible = !sind_el[0].visible;
		}
	} //---------------------------------------------------;
	...
	/**
		This is to determine the X position if the Items relative to List Start
	...
	**/
	function tween_allSlots(Alpha:Array<Float>, StartOffs:String, EndOffs:String, times:String, onComplete:Void->Void, ease:String, hardstart:Bool) {
		...
		while (itemSlots[i] != null) {
			...
			_itm.alpha = Alpha[0];
			tween_slot.push(FlxTween.tween(itemSlots[i], {x: sx + off1[0], y: sy + off1[1], alpha: Alpha[1]}, tt[0], {startDelay: (i * tt[1]), ease: easeFn}));
			i++;
		} // --
	...
	**/
	public function poolSet(mode:String = "keep", size:Int = DEFAULT_POOL_MAX) {
		pool_keep = (mode == "keep");
		pool_max = size;
	} //---------------------------------------------------;
	...
	function poolPut(el:T) {
		...
		// Do I need this one??
		if (pool_keep && pool.indexOf(el) >= 0)
			return;
