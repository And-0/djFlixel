...
class MItemList extends MItem {
	...
	**/
	public function new(MP:MPage, check34:Bool = false) {
		...
		if (mp.styleIt.ar_offy != null)
			offy = mp.styleIt.ar_offy;
		...
		if (mp.styleIt.ar_bm != null) {
			if (check34 && mp.styleIt.ar_bm[3] != null)
				o = 2;
			...
			if (check34 && mp.styleIt.ar_txt[3] != null)
				o = 2;
			if (mp.styleIt.ar_txt == null)
				throw "Style error, you must set text or bitmap";
	...
	override function handleInput(inp:ListItemInput) {
		...
		if (inp == right) {
			...
			// DRJ: Don't automatically fire fire event when FORCE data is set
			if (data.data.force != null && data.data.force == "true") {
		...
		if (inp == left) {
			...
			if (c < 0) {
				if (data.data.loop)
					c = cast data.data.list.length - 1;
				else
					return;
			}
			data.data.c = c;
