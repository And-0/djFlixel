...
typedef MItemStyle = {
	...
	// Text style, to use on all ITEMS
	// All Items will use colors from (col_t,col_b)
	// MUST SET : bt
	col_t:StateColors,
	// Text Colors. You should set ALL states
	col_b:StateColors,
	// Border Colors. You can only set 'idle' and all states will get that color.
	// :: ADVANCED ::
	part2_pad:Int,
	// Push the second part of items (toggle,list,range) to the right by this many pixels
	...
	?box_txt:Array<String>,
	// Text instead of graphic e.g. '[ ]' '[x]'
	?box_bm:Array<BitmapData>,
	// off,on checkbox Bitmaps
	?box_offy:Int,
	// In case of checkbox bitmap icon, offset it this much on the y axis (def=1)
	...
	?ar_txt:Array<String>,
	// Text instead of graphic e.g. '<' '>'
	?ar_bm:Array<BitmapData>,
	// Left,Right Arrows Bitmaps
	?ar_offy:Int,
	// In case of arrow bitmap icon, offset it this much on the y axis (def=1)
	?ar_anim:String,
	// "type,steps,time" Type=1 repat, 2 loop. Every steps is a pixel moved. Time for each tick
	// <defaults in MItemList>
	?bm_no_col:Bool,
	// If true will NOT colorize the bitmaps in [ar_bm, box_bm] use this for custom pre-colored graphics
};
...
class MItem extends FlxSpriteGroup implements IListItem<MItemData> {
	...
	public function new(MP:MPage) {
		...
		#if debug
		if (mp.iconcache == null)
			throw "Forgot to initialize ICONCACHE";
		#end
