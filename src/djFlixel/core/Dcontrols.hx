...
class Dcontrols {
	...
	var PAD_KEYS:Array<Array<FlxGamepadInputID>>;
	...
	**/
	...
	/** Replace all keys with these new ones */
	public function padmap_set_multiple(KEYS:Array<Array<Int>>) {
		if (PAD_KEYS == null)
			PAD_KEYS = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], []];
		trace("PAD Before: " + PAD_KEYS);
		for (i in 0...PAD_KEYS.length) {
			if (i >= KEYS.length)
				PAD_KEYS[i] = []; // continue;//MAP_KEYS[i] = MAP_KEYS_DEFAULT[i];
			else
				PAD_KEYS[i] = KEYS[i];
		}
		trace("PAD After: " + PAD_KEYS);
	...
	**/
	...
	function _padPressed(id:DButton):Bool {
		return switch (id) {
			...
			case LEFT: gp.DPAD_LEFT || gamepad.analog.value.LEFT_STICK_X < 0;
			case RIGHT: gp.DPAD_RIGHT || gamepad.analog.value.LEFT_STICK_X > 0;
			case A: gp.A;
			case X: gp.X;
			case Y: gp.Y;
			case B: gp.B;
	...
	function _padJustPressed(id:DButton):Bool {
		return switch (id) {
			...
			case LEFT: gjp.DPAD_LEFT || (gamepad.analog.justMoved.LEFT_STICK_X && gamepad.analog.value.LEFT_STICK_X < 0);
			case RIGHT: gjp.DPAD_RIGHT || (gamepad.analog.justMoved.LEFT_STICK_X && gamepad.analog.value.LEFT_STICK_X > 0);
			case A: gjp.A;
			case X: gjp.X;
			case Y: gjp.Y;
			case B: gjp.B;
	...
	function _padJustReleased(id:DButton):Bool {
		return gamepad.anyJustReleased(PAD_KEYS[cast id]);
		return switch (id) {
			case UP: gjr.DPAD_UP || gamepad.analog.justReleased.LEFT_STICK_Y;
			case DOWN: gjr.DPAD_DOWN || gamepad.analog.justReleased.LEFT_STICK_Y;
			case LEFT: gjr.DPAD_LEFT || gamepad.analog.justReleased.LEFT_STICK_X;
			case RIGHT: gjr.DPAD_RIGHT || gamepad.analog.justReleased.LEFT_STICK_X;
			case A: gjr.A;
			case X: gjr.X;
			case Y: gjr.Y;
			case B: gjr.B;
	...
	public function generateSpecialKeys() {
		...
		MAP_KEYS[cast DButton._ANY] = MAP_KEYS[cast START].concat(MAP_KEYS[cast SELECT].concat(MAP_KEYS[cast A].concat(MAP_KEYS[cast B].concat(MAP_KEYS[cast X].concat(MAP_KEYS[cast Y])))));
